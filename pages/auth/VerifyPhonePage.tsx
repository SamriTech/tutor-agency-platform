...
const VerifyPhonePage: React.FC = () => {
    ...
    return (
        <>
            {params.get("path") &&
                <div className="flex w-full rounded-sm bg-warning py-2 px-5 gap-10">
                    <p>You forget to signup before. complete signup now</p>
                    {

                        logout.isPending ? <button className="btn btn-success">Loading</button>
                            : <button onClick={() => logout.mutateAsync} className="btn btn-success">Logout</button>
                    }
                    <button onClick={() => logout.mutateAsync} className="btn btn-success">Logout</button>
                </div>
            }
