  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col">
      <div className="relative">
        <img className="w-full h-40 object-cover" src={tutor?.photo} alt={tutor?.username} />
      ...
      <div className="p-4 flex-grow flex flex-col">
        <h3 className="text-lg font-bold text-neutral-800">{tutor?.first_name} {tutor?.last_name}</h3>
        <p className="text-sm text-neutral-500 mb-2 truncate">{tutor?.tutor_profile?.bio}</p>

        <div className="flex items-center mb-3">
          <RatingStars rating={tutor?.rating} />
          <span className="text-xs text-neutral-500 ml-2">({tutor?.reviews_count} reviews)</span>
        </div>

        <div className="mb-4">
          <p className="text-sm font-semibold text-neutral-700">Teaches:</p>
          <div className="flex flex-wrap gap-1 mt-1">
            {tutor?.tutor_profile?.subject?.slice(0, 2).map(s => (
              <span key={s?.id} className="bg-primary/10 text-primary text-xs font-medium px-2 py-1 rounded-full">{s.name}</span>
            ))}
          </div>
        </div>
        ...
        <div className="mt-auto pt-4 border-t border-neutral-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <p className="text-lg font-bold text-primary">ETB {tutor?.tutor_profile?.hourly_rate}<span className="text-sm font-normal text-neutral-500">/hr</span></p>
          </div>
